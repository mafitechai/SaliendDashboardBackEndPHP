...
class Users
{
    ...
    {
        $query =
            'SELECT id, accessToken, refreshToken, password, role, displayName, photoURL, email, userGroups FROM ' .
            $this->db_table .
            '';
    ...
    public function getEmailsGroup($group)
    {
        $query =
            'SELECT email FROM ' . $this->db_table . ' WHERE userGroups = ?';
        $stmt = $this->conn->prepare($query);
        $stmt->execute([$group]);
